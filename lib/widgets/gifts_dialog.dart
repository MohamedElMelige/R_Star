                    ...
                    InkWell(
                      onTap: selectedIndex == -1
                        ...
                        if (int.parse(sender?.balanceInCoins ?? '0') >=
                                ...
                                if (Provider.of<GiftsProvider>(context, listen: false).sendSuccess) {
                                  FirebaseFirestore.instance
                                      .collection('userDiamonds')
                                      .doc(widget.recieverID.toString())
                                      .get()
                                      .then((value) {
                                    int currentDiamonds = int.parse(value.data()!['diamond']);
                                    FirebaseFirestore.instance
                                        .collection('userDiamonds')
                                        .doc(widget.recieverID.toString())
                                        .set({
                                      'diamond': (currentDiamonds + int.parse(gifts[selectedIndex].valueInCoins ?? '0')).toString()
                                    });
                                  });
                                  ...
                                  if (int.parse(gifts[selectedIndex].valueInCoins.toString()) >= 80) {
                                    // if value is bigger than 2000 we add it to global text to show it
                                    if (int.parse(gifts[selectedIndex].valueInCoins.toString()) >= 2000) {
                                    ...
                                    Provider.of<GiftsProvider>(context, listen: false).updateSendSuccuess(false);
                                    Get.back();
                                    DatabaseReference newCommentText = ref.child('comments').push();

                                    await newCommentText.set(json.encode({
                                      'id': sender?.id,
                                      'type': 'GIFT',
                                      if (gifts[selectedIndex].videoLink != null)
                                        'video': gifts[selectedIndex].videoLink,
                                      if (gifts[selectedIndex].image != null)
                                        'image': gifts[selectedIndex].image,
                                      'title': gifts[selectedIndex].title ?? 'GIFT',
                                      'name': sender?.name ?? '',
                                      'email': sender?.email ?? '',
                                    ...
                                    DatabaseReference newComment = ref.child('fullscreenvideo').push();
