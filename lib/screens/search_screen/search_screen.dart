      ...
      body: ChangeNotifierProvider(
        ...
        child: Column(
          children: [
                        ...
                        return InkWell(
                          onTap: () async {
                            ...
                            await value.getUserById(_searchContorller.text,
                                'Bearer ' + (apiToken ?? ''));
                            if (value.user?.data == null||value.user?.data?.length==0) {
                              customSnackBar(
                                  text: 'invalid id'.tr,
                                  context: context,
                              );
            ...
            Consumer<SearchProvider>(
              builder: (context, value, child) {
                if (value.user?.data != null &&value.user?.data?.length!=0&& _searchContorller.text != '')
                  return InkWell(
                    onTap: (){
                      Get.bottomSheet(UserInfoBottomSheet(user: User.fromJson(value.user?.data?.first.toJson()??Map()), recieverID: value.user?.data?.first.id.toString()??'',));
                    },
                    child: Container(
                        ...
                        children: [
                          CircleAvatar(
                            ...
                            backgroundImage: NetworkImage(
                                value.user?.data?.first.image?? 'https://www.testingtime.com/app/uploads/2020/12/in-house_research_2.png'),
                          ),
                          SizedBox(
                            width: 15,
                          ),
