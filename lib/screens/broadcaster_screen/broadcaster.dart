...
class _BroadCasterState extends State<BroadCaster> with WidgetsBindingObserver,SingleTickerProviderStateMixin{
  AgoraClient? client;
  var ref;
  Data? user;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    print(state.name);
    if (state == AppLifecycleState.detached ) resetData();
    if(state == AppLifecycleState.inactive)
      Future.delayed(Duration(minutes: 1)).then((value) => resetData());
  ...
  void initState() {
    ...
    WidgetsBinding.instance!.addPostFrameCallback((timeStamp) async {
      ...
      context.read<BroadCastAudienceProvider>().uploadCurrentDiamonds(arguments['channel'], arguments['user']['total_received_gifts'].toString());
  ...
  resetData() async {
    try
        {
          if(context.read<BroadCastAudienceProvider>().videoPlayerController!=null&&context.read<BroadCastAudienceProvider>().videoPlayerController!.value.isInitialized)
                  ...
                  child: Stack(
                    ...
                    children: [
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            TopBarWidget(
                              ...
                              recieverID: user!.id,
                              type: user!.type,
                              users: context.watch<BroadCastAudienceProvider>().users,
                              arguments: arguments,
                              onIconPressed: () {
                                resetData();
                                Get.back();
                            ...
                            // comments List
                            CommentsList(
                              comments: context.watch<BroadCastAudienceProvider>().comments,
                              recieverID: user!.id,
                            ),
                      ...
                      if(context.watch<BroadCastAudienceProvider>().showImage)
                          Center(
                              child: Image.network(
                                context.watch<BroadCastAudienceProvider>().imageLink,
                                width: double.maxFinite,
                                height: Get.height,
                              ),
                          ),
